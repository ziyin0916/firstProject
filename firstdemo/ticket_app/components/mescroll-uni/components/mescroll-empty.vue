...
<template>
	<view class="mescroll-empty" :class="{ 'empty-fixed': option.fixed }" :style="{ 'z-index': option.zIndex, top: option.top }">
		...
		<view v-if="tipDesc" class="empty-tip-desc">{{ tipDesc }}</view>
...
export default {
	...
	computed: {
		...
		// 文本描述提示
		tipDesc() {
			return this.option.tipDesc == null ? GlobalOption.up.empty.tipDesc : this.option.tipDesc; // 此处不使用短路求值, 用于支持传空串不显示文本提示
		}
	},
	methods: {
		// 点击按钮
